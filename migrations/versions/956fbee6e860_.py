...
Revision ID: 956fbee6e860
Revises: 865d3e0a7d5e
Create Date: 2019-10-12 19:17:31.996285
...
revision = '956fbee6e860'
down_revision = '865d3e0a7d5e'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    sa.Column('id_card', sa.LargeBinary(length=4294967295), nullable=False),
    sa.Column('proof_of_address', sa.LargeBinary(length=4294967295), nullable=False),
    sa.Column('salary', sa.LargeBinary(length=4294967295), nullable=False),
    ...
    sa.Column('amount_under_threshold', sa.Float(precision=12, asdecimal=2), nullable=True),
    ...
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('account_id', sa.Integer(), nullable=True),
    sa.Column('benefits_check_date', sa.DateTime(), nullable=True),
    sa.Column('benefits_threshold_attime', sa.Float(precision=12, asdecimal=2), nullable=True),
    sa.Column('balance_attime', sa.Float(precision=12, asdecimal=2), nullable=True),
    sa.Column('amount_over_threshold', sa.Float(precision=12, asdecimal=2), nullable=True),
    sa.ForeignKeyConstraint(['account_id'], ['account.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_paid_account_benefit_history_account_id'), 'paid_account_benefit_history', ['account_id'], unique=False)
    op.create_index(op.f('ix_paid_account_benefit_history_benefits_check_date'), 'paid_account_benefit_history', ['benefits_check_date'], unique=False)
    op.create_table('transaction_history',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('operation_date', sa.DateTime(), nullable=True),
    sa.Column('account_id', sa.Integer(), nullable=False),
    sa.Column('type', sa.Enum('CREDIT', 'CREDIT_BENEFIT', 'DEBIT', 'DEBIT_AGIOS', name='typetransaction'), server_default='CREDIT', nullable=False),
    sa.Column('operation_amount', sa.Float(precision=12, asdecimal=2), nullable=True),
    sa.Column('balance_before_transaction', sa.Float(precision=12, asdecimal=2), nullable=True),
    sa.Column('balance_attime', sa.Float(precision=12, asdecimal=2), nullable=True),
    sa.ForeignKeyConstraint(['account_id'], ['account.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
...
def downgrade():
    ...
    op.drop_index(op.f('ix_paid_account_benefit_history_benefits_check_date'), table_name='paid_account_benefit_history')
