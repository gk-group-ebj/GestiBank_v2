...
class User(db.Model, PaginatedAPIMixin, UserMixin):
    ...
    def password_hash(self, password):
        if (self._password is None) or (UserPassword.verify_old_password(self.id, password) is False):
...
def verify_reset_password_token(token):
    dict_token = dict()
    try:
        dict_token = decode(token, current_app.config['SECRET_KEY'], algorithm=['HS256'])
    except Exception as e:
        pass
    finally:
        p_id = dict_token['reset_mdp']
